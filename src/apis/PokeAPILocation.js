...
const getLocation = (json) => {
    ...
    return {
        id: json.id,
        name: json.names.filter(function(item) {return (item.language.name === "en");}).map(item => item.name)[0],
        areas: areaList,
        coords: {
            x: "", // TODO : a randomiser
            y: ""
...
export const getAllLocations = async () => {
    ...
    try {
        while (count === -1 || offset < count) {
            console.log(`[GET] : /location (limit : ${limit}, offset : ${offset})`)
            const response = await fetch(`${POKE_API_WS_BASE_URL}/location?offset=${offset}&limit=${limit}`);
            const json = await response.json();

            idList = [...idList, ...json.results.map(item => getLocationId(item))];
