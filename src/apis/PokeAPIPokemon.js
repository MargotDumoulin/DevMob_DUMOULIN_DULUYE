import {POKE_API_WS_BASE_URL, POKE_API_WS_LIMIT} from "../config/PokeAPIConfig";

const getPokemonBaseStat = (stats, name) => {
    return stats.filter(function(item) {return (item.stat.name === name);}).map(item => item.base_stat)[0];
}
const getPokemonId = (json) => {
    return Number(json.url.substring(34, json.url.lastIndexOf('/')));
}
const getArea = (json) => {
    return Number(json.location_area.url.substring(40, json.location_area.url.lastIndexOf('/')));
}
...
export const getAllPokemon = async () => {
    ...
    try {
        while (count === -1 || offset < count) {
            console.log(`[GET] : /pokemon (limit : ${limit}, offset : ${offset})`)
            const response = await fetch(`${POKE_API_WS_BASE_URL}/pokemon?offset=${offset}&limit=${limit}`);
            const json = await response.json();

            idList = [...idList, ...json.results.map(item => getPokemonId(item))];
...
export const getPokemonById = async (id) => {
    try {
        ...
        return getPokemon(await response.json(), await getPokemonEncounterById(id));
    } catch (error) {
        console.error(error);
    }
}
export const getPokemonEncounterById = async (id) => {
    try {
        ...
        const response = await fetch(`${POKE_API_WS_BASE_URL}/pokemon/${id}/encounters`);
